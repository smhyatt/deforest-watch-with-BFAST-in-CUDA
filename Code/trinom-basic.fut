...
let trinomialSingle [ycCount]
  ...
  -- `n = h - 1`, where `h` is the height of the tree in the paper
  ...
  -- `Qlen` is the width of the tree, denoted `w` in the paper
  let Qlen = 2 * m + 1
  let Q = map (\i -> if i == m then one else zero) (iota Qlen)

  let alphas = replicate (n + 1) zero
  let alpha_i= (h_YieldCurve[0]).P
  let alphas[0] = alpha_i 
  
  -- time stepping
  let (_,_,alphas) =
    loop (Q: *[Qlen]real, alpha_i: real, alphas: *[]real) for i < n do
      ...
      let Q' = -- 1. result of size independent of i (hoistable)
              map (\j -> if (j < (-imax)) || (j > imax)
                         then zero
                         else fwdHelper M dr dt alpha_i Q m i imax jmax j
                  ) (map (\a->a-m) (iota Qlen))
      ...
      let P = r_exp(-R*t)
      let alpha_i' = r_log (alpha_val / P)
      let alphas[i + 1] = alpha_i'
