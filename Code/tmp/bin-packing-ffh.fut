...
let sgmPrefSum [n] (flags: [n]i32) (data: [n]i32) : [n]i32 =
  let (_,r) = (unzip (scan (\(x_flag,x) (y_flag,y) ->
...
--          let all_data = scatter (copy all_data) inds_v data
            ...
            in  (num_moves, all_perm, all_data, cur_shape, true, count+1i32)
...
let main [arr_len] (arr : [arr_len]i32) : ([]i32, []i32, i32, i32, f32, bool) = 
  ...
  loop ((check_perm,ok)) for i < arr_len do
      ...
      let check_perm[perm_ind] = 1i32
      in  (check_perm,ok)

  in  (perm, binpack_shape, reduce (+) 0 binpack_shape, w, r32(waste)/r32(num_sgms*w), ok)
