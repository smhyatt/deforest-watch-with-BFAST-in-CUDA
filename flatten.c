...
static void futrts_cli_entry_main(struct futhark_context *ctx)
{
    ...
    int32_t read_value_4629;
    
    if (read_scalar(&i32_info, &read_value_4629) != 0)
        panic(1, "Error when reading input #%d of type %s (errno: %s).\n", 0,
              i32_info.type_name, strerror(errno));
    
    int32_t read_value_4630;
    
    if (read_scalar(&i32_info, &read_value_4630) != 0)
        panic(1, "Error when reading input #%d of type %s (errno: %s).\n", 1,
              i32_info.type_name, strerror(errno));
    
    int32_t read_value_4631;
    
    if (read_scalar(&i32_info, &read_value_4631) != 0)
        panic(1, "Error when reading input #%d of type %s (errno: %s).\n", 2,
              i32_info.type_name, strerror(errno));
    
    float read_value_4632;
    
    if (read_scalar(&f32_info, &read_value_4632) != 0)
        panic(1, "Error when reading input #%d of type %s (errno: %s).\n", 3,
              f32_info.type_name, strerror(errno));
    
    float read_value_4633;
    
    if (read_scalar(&f32_info, &read_value_4633) != 0)
        panic(1, "Error when reading input #%d of type %s (errno: %s).\n", 4,
              f32_info.type_name, strerror(errno));
    
    float read_value_4634;
    
    if (read_scalar(&f32_info, &read_value_4634) != 0)
        panic(1, "Error when reading input #%d of type %s (errno: %s).\n", 5,
              f32_info.type_name, strerror(errno));
    
    struct futhark_i32_1d *read_value_4635;
    int64_t read_shape_4636[1];
    int32_t *read_arr_4637 = NULL;
    
    errno = 0;
    if (read_array(&i32_info, (void **) &read_arr_4637, read_shape_4636, 1) !=
        0)
        panic(1, "Cannot read input #%d of type %s%s (errno: %s).\n", 6, "[]",
              i32_info.type_name, strerror(errno));
    
    struct futhark_f32_2d *read_value_4638;
    int64_t read_shape_4639[2];
    float *read_arr_4640 = NULL;
    
    errno = 0;
    if (read_array(&f32_info, (void **) &read_arr_4640, read_shape_4639, 2) !=
        0)
        panic(1, "Cannot read input #%d of type %s%s (errno: %s).\n", 7, "[][]",
              f32_info.type_name, strerror(errno));
    
    struct futhark_i32_1d *result_4641;
    struct futhark_f32_1d *result_4642;
    ...
    if (perform_warmup) {
        ...
        ;
        ;
        assert((read_value_4635 = futhark_new_i32_1d(ctx, read_arr_4637,
                                                     read_shape_4636[0])) != 0);
        assert((read_value_4638 = futhark_new_f32_2d(ctx, read_arr_4640,
                                                     read_shape_4639[0],
                                                     read_shape_4639[1])) != 0);
        assert(futhark_context_sync(ctx) == 0);
        t_start = get_wall_time();
        r = futhark_entry_main(ctx, &result_4641, &result_4642, read_value_4629,
                               read_value_4630, read_value_4631,
                               read_value_4632, read_value_4633,
                               read_value_4634, read_value_4635,
                               read_value_4638);
        if (r != 0)
        ...
        ;
        ;
        assert(futhark_free_i32_1d(ctx, read_value_4635) == 0);
        assert(futhark_free_f32_2d(ctx, read_value_4638) == 0);
        assert(futhark_free_i32_1d(ctx, result_4641) == 0);
        assert(futhark_free_f32_1d(ctx, result_4642) == 0);
    }
    time_runs = 1;
    /* Proper run. */
        ...
        set_binary_mode(stdout);
    {
        ...
        assert(arr != NULL);
        assert(futhark_values_i32_1d(ctx, result_4641, arr) == 0);
        write_array(stdout, binary_output, &i32_info, arr,
                    futhark_shape_i32_1d(ctx, result_4641), 1);
        free(arr);
    }
    printf("\n");
    {
        ...
        assert(arr != NULL);
        assert(futhark_values_f32_1d(ctx, result_4642, arr) == 0);
        write_array(stdout, binary_output, &f32_info, arr,
                    futhark_shape_f32_1d(ctx, result_4642), 1);
        free(arr);
    }
    printf("\n");
    assert(futhark_free_i32_1d(ctx, result_4641) == 0);
...
const char *opencl_program[] =
           {"#ifdef cl_clang_storage_class_specifiers\n#pragma OPENCL EXTENSION cl_clang_storage_class_specifiers : enable\n#endif\n#pragma OPENCL EXTENSION cl_khr_byte_addressable_store : enable\n__kernel void dummy_kernel(__global unsigned char *dummy, int n)\n{\n    const int thread_gid = get_global_id(0);\n    \n    if (thread_gid >= n)\n        return;\n}\ntypedef char int8_t;\ntypedef short int16_t;\ntypedef int int32_t;\ntypedef long int64_t;\ntypedef uchar uint8_t;\ntypedef ushort uint16_t;\ntypedef uint uint32_t;\ntypedef ulong uint64_t;\n#define ALIGNED_LOCAL_MEMORY(m,size) __local int64_t m[((size + 7) & ~7)/8]\n#ifdef cl_nv_pragma_unroll\nstatic inline void mem_fence_global()\n{\n    asm(\"membar.gl;\");\n}\n#else\nstatic inline void mem_fence_global()\n{\n    mem_fence(CLK_LOCAL_MEM_FENCE | CLK_GLOBAL_MEM_FENCE);\n}\n#endif\nstatic inline void mem_fence_local()\n{\n    mem_fence(CLK_LOCAL_MEM_FENCE);\n}\nstatic inline int8_t add8(int8_t x, int8_t y)\n{\n    return x + y;\n}\nstatic inline int16_t add16(int16_t x, int16_t y)\n{\n    return x + y;\n}\nstatic inline int32_t add32(int32_t x, int32_t y)\n{\n    return x + y;\n}\nstatic inline int64_t add64(int64_t x, int64_t y)\n{\n    return x + y;\n}\nstatic inline int8_t sub8(int8_t x, int8_t y)\n{\n    return x - y;\n}\nstatic inline int16_t sub16(int16_t x, int16_t y)\n{\n    return x - y;\n}\nstatic inline int32_t sub32(int32_t x, int32_t y)\n{\n    return x - y;\n}\nstatic inline int64_t sub64(int64_t x, int64_t y)\n{\n    return x - y;\n}\nstatic inline int8_t mul8(int8_t x, int8_t y)\n{\n    return x * y;\n}\nstatic inline int16_t mul16(int16_t x, int16_t y)\n{\n    return x * y;\n}\nstatic inline int32_t mul32(int32_t x, int32_t y)\n{\n    return x * y;\n}\nstatic inline int64_t mul64(int64_t x, int64_t y)\n{\n    return x * y;\n}\nstatic inline uint8_t udiv8(uint8_t x, uint8_t y)\n{\n    return x / y;\n}\nstatic inline uint16_t udiv16(uint16_t x, uint16_t y)\n{\n    return x / y;\n}\nstatic inline uint32_t udiv32(uint32_t x, uint32_t y)\n{\n    return x / y;\n}\nstatic inline uint64_t udiv64(uint64_t x, u",
            ...
            "nf32(float x)\n{\n    return isinf(x);\n}\nstatic inline int32_t futrts_to_bits32(float x)\n{\n    union {\n        float f;\n        int32_t t;\n    } p;\n    \n    p.f = x;\n    return p.t;\n}\nstatic inline float futrts_from_bits32(int32_t x)\n{\n    union {\n        int32_t f;\n        float t;\n    } p;\n    \n    p.f = x;\n    return p.t;\n}\n__kernel void segmap_4586(int32_t N_4536, int32_t N_4538, __global\n                          unsigned char *images_mem_4603, __global\n                          unsigned char *mem_4608)\n{\n    const int32_t segmap_group_sizze_4590 = mainzisegmap_group_sizze_4589;\n    const int block_dim0 = 0;\n    const int block_dim1 = 1;\n    const int block_dim2 = 2;\n    int32_t global_tid_4615;\n    int32_t local_tid_4616;\n    int32_t group_sizze_4619;\n    int32_t wave_sizze_4618;\n    int32_t group_tid_4617;\n    \n    global_tid_4615 = get_global_id(0);\n    local_tid_4616 = get_local_id(0);\n    group_sizze_4619 = get_local_size(0);\n    wave_sizze_4618 = LOCKSTEP_WIDTH;\n    group_tid_4617 = get_group_id(0);\n    \n    int32_t phys_tid_4586 = global_tid_4615;\n    int32_t gtid_4584 = squot32(group_tid_4617 * segmap_group_sizze_4590 +\n                                local_tid_4616, N_4536);\n    int32_t gtid_4585;\n    \n    gtid_4585 = group_tid_4617 * segmap_group_sizze_4590 + local_tid_4616 -\n        squot32(group_tid_4617 * segmap_group_sizze_4590 + local_tid_4616,\n                N_4536) * N_4536;\n    if (slt32(gtid_4584, 2) && slt32(gtid_4585, N_4536)) {\n        float x_elem_4594 = ((__global float *) images_mem_4603)[gtid_4584 *\n                                                                 N_4538 +\n                                                                 gtid_4585];\n        bool res_4595;\n        \n        res_4595 = futrts_isnan32(x_elem_4594);\n        \n        float res_4596;\n        \n        if (res_4595) {\n            res_4596 = -3.4028235e38F;\n        } else {\n            res_4596 = x_elem_4594;\n        }\n        ((__global float *) mem_4608)[gtid_458",
            "4 * N_4536 + gtid_4585] =\n            res_4596;\n    }\n}\n",
            NULL};
...
static const char *size_names[] = {"main.segmap_group_size_4589"};
static const char *size_vars[] = {"mainzisegmap_group_sizze_4589"};
...
struct sizes {
    size_t mainzisegmap_group_sizze_4589;
} ;
struct futhark_context_config {
                             ...
                             struct futhark_context *ctx, cl_program prog)
{
    ...
    ctx->sizes.mainzisegmap_group_sizze_4589 = cfg->sizes[0];
    return 0;
}
struct futhark_context *futhark_context_new(struct futhark_context_config *cfg)
{
...
static int futrts_main(struct futhark_context *ctx,
                       struct memblock_device *out_mem_p_4620,
                       int32_t *out_out_arrsizze_4621,
                       struct memblock_device *out_mem_p_4622,
                       int32_t *out_out_arrsizze_4623,
                       struct memblock_device mappingindices_mem_4602,
                       struct memblock_device images_mem_4603, int32_t N_4536,
...
static int futrts_main(struct futhark_context *ctx,
                       ...
                       float hfrac_4543, float lam_4544)
{
    struct memblock_device out_mem_4611;
    
    out_mem_4611.references = NULL;
    
    int32_t out_arrsizze_4612;
    struct memblock_device out_mem_4613;
    
    out_mem_4613.references = NULL;
    
    int32_t out_arrsizze_4614;
    ...
    if (!empty_or_match_4554) {
        ...
        if (memblock_unref_device(ctx, &out_mem_4613, "out_mem_4613") != 0)
            return 1;
        if (memblock_unref_device(ctx, &out_mem_4611, "out_mem_4611") != 0)
            return 1;
        return 1;
    }
    ...
    int32_t nest_sizze_4588 = 2 * N_4536;
    int32_t segmap_group_sizze_4590;
    
    segmap_group_sizze_4590 = ctx->sizes.mainzisegmap_group_sizze_4589;
    
    int32_t y_4591 = segmap_group_sizze_4590 - 1;
    int32_t x_4592 = nest_sizze_4588 + y_4591;
    int32_t segmap_usable_groups_4593 = squot32(x_4592,
                                                segmap_group_sizze_4590);
    int64_t binop_y_4606 = sext_i32_i64(N_4536);
    int64_t binop_x_4607 = 2 * binop_y_4606;
    int64_t bytes_4604 = 4 * binop_x_4607;
    struct memblock_device mem_4608;
    
    mem_4608.references = NULL;
    if (memblock_alloc_device(ctx, &mem_4608, bytes_4604, "mem_4608"))
        return 1;
    ...
    OPENCL_SUCCEED_OR_RETURN(clSetKernelArg(ctx->segmap_4586, 2,
                                            sizeof(images_mem_4603.mem),
                                            &images_mem_4603.mem));
    OPENCL_SUCCEED_OR_RETURN(clSetKernelArg(ctx->segmap_4586, 3,
                                            sizeof(mem_4608.mem),
                                            &mem_4608.mem));
    if (1 * (segmap_usable_groups_4593 * segmap_group_sizze_4590) != 0) {
        const size_t global_work_sizze_4624[1] = {segmap_usable_groups_4593 *
                     segmap_group_sizze_4590};
        const size_t local_work_sizze_4628[1] = {segmap_group_sizze_4590};
        int64_t time_start_4625 = 0, time_end_4626 = 0;
        
        if (ctx->debugging) {
            fprintf(stderr, "Launching %s with global work size [",
                    "segmap_4586");
            fprintf(stderr, "%zu", global_work_sizze_4624[0]);
            fprintf(stderr, "] and local work size [");
            fprintf(stderr, "%zu", local_work_sizze_4628[0]);
            fprintf(stderr, "].\n");
            time_start_4625 = get_wall_time();
        }
        OPENCL_SUCCEED_OR_RETURN(clEnqueueNDRangeKernel(ctx->opencl.queue,
                                                        ctx->segmap_4586, 1,
                                                        NULL,
                                                        global_work_sizze_4624,
                                                        local_work_sizze_4628,
                                                        0, NULL, NULL));
        if (ctx->debugging) {
            OPENCL_SUCCEED_FATAL(clFinish(ctx->opencl.queue));
            time_end_4626 = get_wall_time();
            
            long time_diff_4627 = time_end_4626 - time_start_4625;
            
            ctx->segmap_4586_total_runtime += time_diff_4627;
            ctx->segmap_4586_runs++;
            fprintf(stderr, "kernel %s runtime: %ldus\n", "segmap_4586",
                    time_diff_4627);
        }
    }
    out_arrsizze_4612 = N_4536;
    out_arrsizze_4614 = nest_sizze_4588;
    if (memblock_set_device(ctx, &out_mem_4611, &mappingindices_mem_4602,
                            "mappingindices_mem_4602") != 0)
        return 1;
    if (memblock_set_device(ctx, &out_mem_4613, &mem_4608, "mem_4608") != 0)
        return 1;
    (*out_mem_p_4620).references = NULL;
    if (memblock_set_device(ctx, &*out_mem_p_4620, &out_mem_4611,
                            "out_mem_4611") != 0)
        return 1;
    *out_out_arrsizze_4621 = out_arrsizze_4612;
    (*out_mem_p_4622).references = NULL;
    if (memblock_set_device(ctx, &*out_mem_p_4622, &out_mem_4613,
                            "out_mem_4613") != 0)
        return 1;
    *out_out_arrsizze_4623 = out_arrsizze_4614;
    if (memblock_unref_device(ctx, &mem_4608, "mem_4608") != 0)
        return 1;
    if (memblock_unref_device(ctx, &out_mem_4613, "out_mem_4613") != 0)
        return 1;
    if (memblock_unref_device(ctx, &out_mem_4611, "out_mem_4611") != 0)
                       ...
                       struct futhark_f32_2d *in7)
{
    ...
    mappingindices_mem_4602.references = NULL;
    
    struct memblock_device images_mem_4603;
    
    images_mem_4603.references = NULL;
    ...
    hfrac_4543 = in4;
    lam_4544 = in5;
    mappingindices_mem_4602 = in6->mem;
    N_4536 = in6->shape[0];
    images_mem_4603 = in7->mem;
    m_4537 = in7->shape[0];
    N_4538 = in7->shape[1];
    
    int ret = futrts_main(ctx, &out_mem_4611, &out_arrsizze_4612, &out_mem_4613,
                          &out_arrsizze_4614, mappingindices_mem_4602,
                          images_mem_4603, N_4536, m_4537, N_4538, trend_4539,
    ...
    if (ret == 0) {
        assert((*out0 =
                (struct futhark_i32_1d *) malloc(sizeof(struct futhark_i32_1d))) !=
            NULL);
        (*out0)->mem = out_mem_4611;
        (*out0)->shape[0] = out_arrsizze_4612;
        assert((*out1 =
                (struct futhark_f32_1d *) malloc(sizeof(struct futhark_f32_1d))) !=
            NULL);
        (*out1)->mem = out_mem_4613;
        (*out1)->shape[0] = out_arrsizze_4614;
    }
    lock_unlock(&ctx->lock);
    return ret;
}
