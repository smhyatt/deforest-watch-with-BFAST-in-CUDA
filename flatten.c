...
static void futrts_cli_entry_main(struct futhark_context *ctx)
{
    ...
    int32_t read_value_4689;
    
    if (read_scalar(&i32_info, &read_value_4689) != 0)
        panic(1, "Error when reading input #%d of type %s (errno: %s).\n", 0,
              i32_info.type_name, strerror(errno));
    
    int32_t read_value_4690;
    
    if (read_scalar(&i32_info, &read_value_4690) != 0)
        panic(1, "Error when reading input #%d of type %s (errno: %s).\n", 1,
              i32_info.type_name, strerror(errno));
    
    int32_t read_value_4691;
    
    if (read_scalar(&i32_info, &read_value_4691) != 0)
        panic(1, "Error when reading input #%d of type %s (errno: %s).\n", 2,
              i32_info.type_name, strerror(errno));
    
    float read_value_4692;
    
    if (read_scalar(&f32_info, &read_value_4692) != 0)
        panic(1, "Error when reading input #%d of type %s (errno: %s).\n", 3,
              f32_info.type_name, strerror(errno));
    
    float read_value_4693;
    
    if (read_scalar(&f32_info, &read_value_4693) != 0)
        panic(1, "Error when reading input #%d of type %s (errno: %s).\n", 4,
              f32_info.type_name, strerror(errno));
    
    float read_value_4694;
    
    if (read_scalar(&f32_info, &read_value_4694) != 0)
        panic(1, "Error when reading input #%d of type %s (errno: %s).\n", 5,
              f32_info.type_name, strerror(errno));
    
    struct futhark_i32_1d *read_value_4695;
    int64_t read_shape_4696[1];
    int32_t *read_arr_4697 = NULL;
    
    errno = 0;
    if (read_array(&i32_info, (void **) &read_arr_4697, read_shape_4696, 1) !=
        0)
        panic(1, "Cannot read input #%d of type %s%s (errno: %s).\n", 6, "[]",
              i32_info.type_name, strerror(errno));
    
    struct futhark_f32_2d *read_value_4698;
    int64_t read_shape_4699[2];
    float *read_arr_4700 = NULL;
    
    errno = 0;
    if (read_array(&f32_info, (void **) &read_arr_4700, read_shape_4699, 2) !=
        0)
        panic(1, "Cannot read input #%d of type %s%s (errno: %s).\n", 7, "[][]",
              f32_info.type_name, strerror(errno));
    ...
    float result_4705;
    float result_4706;
    struct futhark_i32_1d *result_4707;
    struct futhark_f32_1d *result_4708;
    ...
    if (perform_warmup) {
        ...
        ;
        ;
        assert((read_value_4695 = futhark_new_i32_1d(ctx, read_arr_4697,
                                                     read_shape_4696[0])) != 0);
        assert((read_value_4698 = futhark_new_f32_2d(ctx, read_arr_4700,
                                                     read_shape_4699[0],
                                                     read_shape_4699[1])) != 0);
        assert(futhark_context_sync(ctx) == 0);
        t_start = get_wall_time();
        r = futhark_entry_main(ctx, &result_4701, &result_4702, &result_4703,
                               &result_4704, &result_4705, &result_4706,
                               &result_4707, &result_4708, read_value_4689,
                               read_value_4690, read_value_4691,
                               read_value_4692, read_value_4693,
                               read_value_4694, read_value_4695,
                               read_value_4698);
        if (r != 0)
        ...
        ;
        ;
        assert(futhark_free_i32_1d(ctx, read_value_4695) == 0);
        assert(futhark_free_f32_2d(ctx, read_value_4698) == 0);
        ;
        ;
        ;
        ;
        ;
        ;
        assert(futhark_free_i32_1d(ctx, result_4707) == 0);
        assert(futhark_free_f32_1d(ctx, result_4708) == 0);
    }
    time_runs = 1;
    /* Proper run. */
    ...
    write_scalar(stdout, binary_output, &i32_info, &result_4701);
    printf("\n");
    write_scalar(stdout, binary_output, &i32_info, &result_4702);
    printf("\n");
    write_scalar(stdout, binary_output, &i32_info, &result_4703);
    printf("\n");
    write_scalar(stdout, binary_output, &f32_info, &result_4704);
    printf("\n");
    write_scalar(stdout, binary_output, &f32_info, &result_4705);
    printf("\n");
    write_scalar(stdout, binary_output, &f32_info, &result_4706);
    printf("\n");
    {
        ...
        assert(arr != NULL);
        assert(futhark_values_i32_1d(ctx, result_4707, arr) == 0);
        write_array(stdout, binary_output, &i32_info, arr,
                    futhark_shape_i32_1d(ctx, result_4707), 1);
        free(arr);
    }
    printf("\n");
    {
        ...
        assert(arr != NULL);
        assert(futhark_values_f32_1d(ctx, result_4708, arr) == 0);
        write_array(stdout, binary_output, &f32_info, arr,
                    futhark_shape_f32_1d(ctx, result_4708), 1);
        free(arr);
    }
    printf("\n");
    ;
...
const char *opencl_program[] =
           {"#ifdef cl_clang_storage_class_specifiers\n#pragma OPENCL EXTENSION cl_clang_storage_class_specifiers : enable\n#endif\n#pragma OPENCL EXTENSION cl_khr_byte_addressable_store : enable\n__kernel void dummy_kernel(__global unsigned char *dummy, int n)\n{\n    const int thread_gid = get_global_id(0);\n    \n    if (thread_gid >= n)\n        return;\n}\ntypedef char int8_t;\ntypedef short int16_t;\ntypedef int int32_t;\ntypedef long int64_t;\ntypedef uchar uint8_t;\ntypedef ushort uint16_t;\ntypedef uint uint32_t;\ntypedef ulong uint64_t;\n#define ALIGNED_LOCAL_MEMORY(m,size) __local int64_t m[((size + 7) & ~7)/8]\n#ifdef cl_nv_pragma_unroll\nstatic inline void mem_fence_global()\n{\n    asm(\"membar.gl;\");\n}\n#else\nstatic inline void mem_fence_global()\n{\n    mem_fence(CLK_LOCAL_MEM_FENCE | CLK_GLOBAL_MEM_FENCE);\n}\n#endif\nstatic inline void mem_fence_local()\n{\n    mem_fence(CLK_LOCAL_MEM_FENCE);\n}\nstatic inline int8_t add8(int8_t x, int8_t y)\n{\n    return x + y;\n}\nstatic inline int16_t add16(int16_t x, int16_t y)\n{\n    return x + y;\n}\nstatic inline int32_t add32(int32_t x, int32_t y)\n{\n    return x + y;\n}\nstatic inline int64_t add64(int64_t x, int64_t y)\n{\n    return x + y;\n}\nstatic inline int8_t sub8(int8_t x, int8_t y)\n{\n    return x - y;\n}\nstatic inline int16_t sub16(int16_t x, int16_t y)\n{\n    return x - y;\n}\nstatic inline int32_t sub32(int32_t x, int32_t y)\n{\n    return x - y;\n}\nstatic inline int64_t sub64(int64_t x, int64_t y)\n{\n    return x - y;\n}\nstatic inline int8_t mul8(int8_t x, int8_t y)\n{\n    return x * y;\n}\nstatic inline int16_t mul16(int16_t x, int16_t y)\n{\n    return x * y;\n}\nstatic inline int32_t mul32(int32_t x, int32_t y)\n{\n    return x * y;\n}\nstatic inline int64_t mul64(int64_t x, int64_t y)\n{\n    return x * y;\n}\nstatic inline uint8_t udiv8(uint8_t x, uint8_t y)\n{\n    return x / y;\n}\nstatic inline uint16_t udiv16(uint16_t x, uint16_t y)\n{\n    return x / y;\n}\nstatic inline uint32_t udiv32(uint32_t x, uint32_t y)\n{\n    return x / y;\n}\nstatic inline uint64_t udiv64(uint64_t x, u",
            ...
            "nf32(float x)\n{\n    return isinf(x);\n}\nstatic inline int32_t futrts_to_bits32(float x)\n{\n    union {\n        float f;\n        int32_t t;\n    } p;\n    \n    p.f = x;\n    return p.t;\n}\nstatic inline float futrts_from_bits32(int32_t x)\n{\n    union {\n        int32_t f;\n        float t;\n    } p;\n    \n    p.f = x;\n    return p.t;\n}\n__kernel void segmap_4634(int32_t N_4584, int32_t N_4586, __global\n                          unsigned char *images_mem_4651, __global\n                          unsigned char *mem_4656)\n{\n    const int32_t segmap_group_sizze_4638 = mainzisegmap_group_sizze_4637;\n    const int block_dim0 = 0;\n    const int block_dim1 = 1;\n    const int block_dim2 = 2;\n    int32_t global_tid_4669;\n    int32_t local_tid_4670;\n    int32_t group_sizze_4673;\n    int32_t wave_sizze_4672;\n    int32_t group_tid_4671;\n    \n    global_tid_4669 = get_global_id(0);\n    local_tid_4670 = get_local_id(0);\n    group_sizze_4673 = get_local_size(0);\n    wave_sizze_4672 = LOCKSTEP_WIDTH;\n    group_tid_4671 = get_group_id(0);\n    \n    int32_t phys_tid_4634 = global_tid_4669;\n    int32_t gtid_4632 = squot32(group_tid_4671 * segmap_group_sizze_4638 +\n                                local_tid_4670, N_4584);\n    int32_t gtid_4633;\n    \n    gtid_4633 = group_tid_4671 * segmap_group_sizze_4638 + local_tid_4670 -\n        squot32(group_tid_4671 * segmap_group_sizze_4638 + local_tid_4670,\n                N_4584) * N_4584;\n    if (slt32(gtid_4632, 2) && slt32(gtid_4633, N_4584)) {\n        float x_elem_4642 = ((__global float *) images_mem_4651)[gtid_4632 *\n                                                                 N_4586 +\n                                                                 gtid_4633];\n        bool res_4643;\n        \n        res_4643 = futrts_isnan32(x_elem_4642);\n        \n        float res_4644;\n        \n        if (res_4643) {\n            res_4644 = -3.4028235e38F;\n        } else {\n            res_4644 = x_elem_4642;\n        }\n        ((__global float *) mem_4656)[gtid_463",
            "2 * N_4584 + gtid_4633] =\n            res_4644;\n    }\n}\n",
            NULL};
...
static const char *size_names[] = {"main.segmap_group_size_4637"};
static const char *size_vars[] = {"mainzisegmap_group_sizze_4637"};
...
struct sizes {
    size_t mainzisegmap_group_sizze_4637;
} ;
struct futhark_context_config {
...
struct futhark_context {
    ...
    int total_runs;
    long total_runtime;
    cl_kernel segmap_4634;
    int segmap_4634_total_runtime;
    int segmap_4634_runs;
                               ...
                               struct futhark_context *ctx)
{
    ...
    ctx->total_runs = 0;
    ctx->total_runtime = 0;
    ctx->segmap_4634_total_runtime = 0;
    ctx->segmap_4634_runs = 0;
}
                             ...
                             struct futhark_context *ctx, cl_program prog)
{
    ...
    {
        ctx->segmap_4634 = clCreateKernel(prog, "segmap_4634", &error);
        OPENCL_SUCCEED_FATAL(error);
        if (ctx->debugging)
            fprintf(stderr, "Created kernel %s.\n", "segmap_4634");
    }
    ctx->sizes.mainzisegmap_group_sizze_4637 = cfg->sizes[0];
    return 0;
}
struct futhark_context *futhark_context_new(struct futhark_context_config *cfg)
{
...
void futhark_debugging_report(struct futhark_context *ctx)
{
    ...
    if (ctx->debugging) {
        fprintf(stderr,
                "Kernel segmap_4634 executed %6d times, with average runtime: %6ldus\tand total runtime: %6ldus\n",
                ctx->segmap_4634_runs, (long) ctx->segmap_4634_total_runtime /
                (ctx->segmap_4634_runs != 0 ? ctx->segmap_4634_runs : 1),
                (long) ctx->segmap_4634_total_runtime);
        ctx->total_runtime += ctx->segmap_4634_total_runtime;
        ctx->total_runs += ctx->segmap_4634_runs;
        if (ctx->debugging)
...
static int futrts_main(struct futhark_context *ctx,
                       int32_t *out_scalar_out_4674,
                       int32_t *out_scalar_out_4675,
                       int32_t *out_scalar_out_4676, float *out_scalar_out_4677,
                       float *out_scalar_out_4678, float *out_scalar_out_4679,
                       struct memblock_device *out_mem_p_4680,
                       int32_t *out_out_arrsizze_4681,
                       struct memblock_device *out_mem_p_4682,
                       int32_t *out_out_arrsizze_4683,
                       struct memblock_device mappingindices_mem_4650,
                       struct memblock_device images_mem_4651, int32_t N_4584,
                       int32_t m_4585, int32_t N_4586, int32_t trend_4587,
                       int32_t k_4588, int32_t n_4589, float freq_4590,
                       float hfrac_4591, float lam_4592);
static inline int8_t add8(int8_t x, int8_t y)
{
    return x + y;
}
...
static int futrts_main(struct futhark_context *ctx,
                       ...
                       float hfrac_4591, float lam_4592)
{
    int32_t scalar_out_4659;
    int32_t scalar_out_4660;
    int32_t scalar_out_4661;
    float scalar_out_4662;
    float scalar_out_4663;
    float scalar_out_4664;
    struct memblock_device out_mem_4665;
    
    out_mem_4665.references = NULL;
    
    int32_t out_arrsizze_4666;
    struct memblock_device out_mem_4667;
    
    out_mem_4667.references = NULL;
    
    int32_t out_arrsizze_4668;
    bool dim_zzero_4595 = 0 == m_4585;
    bool dim_zzero_4596 = 0 == N_4586;
    bool old_empty_4597 = dim_zzero_4595 || dim_zzero_4596;
    bool dim_zzero_4598 = 0 == N_4584;
    bool new_empty_4599 = dim_zzero_4595 || dim_zzero_4598;
    bool both_empty_4600 = old_empty_4597 && new_empty_4599;
    bool dim_match_4601 = N_4584 == N_4586;
    bool empty_or_match_4602 = both_empty_4600 || dim_match_4601;
    bool empty_or_match_cert_4603;
    
    if (!empty_or_match_4602) {
        ctx->error = msgprintf("Error at %s:\n%s\n", "flatten.fut:4:1-17:57",
                               "function arguments of wrong shape");
        if (memblock_unref_device(ctx, &out_mem_4667, "out_mem_4667") != 0)
            return 1;
        if (memblock_unref_device(ctx, &out_mem_4665, "out_mem_4665") != 0)
            return 1;
        return 1;
    }
    
    bool i_p_m_t_s_leq_w_4604 = slt32(1, m_4585);
    bool index_certs_4605;
    
    if (!i_p_m_t_s_leq_w_4604) {
        ctx->error = msgprintf("Error at %s:\n%s%d%s%d%s%d%s\n",
                               "flatten.fut:4:1-17:57 -> flatten.fut:14:7-17",
                               "Index [", 0, ":", 2,
                               "] out of bounds for array of shape [", m_4585,
                               "].");
        if (memblock_unref_device(ctx, &out_mem_4667, "out_mem_4667") != 0)
            return 1;
        if (memblock_unref_device(ctx, &out_mem_4665, "out_mem_4665") != 0)
    ...
    int32_t nest_sizze_4636 = 2 * N_4584;
    int32_t segmap_group_sizze_4638;
    
    segmap_group_sizze_4638 = ctx->sizes.mainzisegmap_group_sizze_4637;
    
    int32_t y_4639 = segmap_group_sizze_4638 - 1;
    int32_t x_4640 = nest_sizze_4636 + y_4639;
    int32_t segmap_usable_groups_4641 = squot32(x_4640,
                                                segmap_group_sizze_4638);
    int64_t binop_y_4654 = sext_i32_i64(N_4584);
    int64_t binop_x_4655 = 2 * binop_y_4654;
    int64_t bytes_4652 = 4 * binop_x_4655;
    struct memblock_device mem_4656;
    
    mem_4656.references = NULL;
    if (memblock_alloc_device(ctx, &mem_4656, bytes_4652, "mem_4656"))
        return 1;
    OPENCL_SUCCEED_OR_RETURN(clSetKernelArg(ctx->segmap_4634, 0, sizeof(N_4584),
                                            &N_4584));
    OPENCL_SUCCEED_OR_RETURN(clSetKernelArg(ctx->segmap_4634, 1, sizeof(N_4586),
                                            &N_4586));
    OPENCL_SUCCEED_OR_RETURN(clSetKernelArg(ctx->segmap_4634, 2,
                                            sizeof(images_mem_4651.mem),
                                            &images_mem_4651.mem));
    OPENCL_SUCCEED_OR_RETURN(clSetKernelArg(ctx->segmap_4634, 3,
                                            sizeof(mem_4656.mem),
                                            &mem_4656.mem));
    if (1 * (segmap_usable_groups_4641 * segmap_group_sizze_4638) != 0) {
        const size_t global_work_sizze_4684[1] = {segmap_usable_groups_4641 *
                     segmap_group_sizze_4638};
        const size_t local_work_sizze_4688[1] = {segmap_group_sizze_4638};
        int64_t time_start_4685 = 0, time_end_4686 = 0;
        
        if (ctx->debugging) {
            fprintf(stderr, "Launching %s with global work size [",
                    "segmap_4634");
            fprintf(stderr, "%zu", global_work_sizze_4684[0]);
            fprintf(stderr, "] and local work size [");
            fprintf(stderr, "%zu", local_work_sizze_4688[0]);
            fprintf(stderr, "].\n");
            time_start_4685 = get_wall_time();
        }
        OPENCL_SUCCEED_OR_RETURN(clEnqueueNDRangeKernel(ctx->opencl.queue,
                                                        ctx->segmap_4634, 1,
                                                        NULL,
                                                        global_work_sizze_4684,
                                                        local_work_sizze_4688,
                                                        0, NULL, NULL));
        if (ctx->debugging) {
            OPENCL_SUCCEED_FATAL(clFinish(ctx->opencl.queue));
            time_end_4686 = get_wall_time();
            
            long time_diff_4687 = time_end_4686 - time_start_4685;
            
            ctx->segmap_4634_total_runtime += time_diff_4687;
            ctx->segmap_4634_runs++;
            fprintf(stderr, "kernel %s runtime: %ldus\n", "segmap_4634",
                    time_diff_4687);
        }
    }
    out_arrsizze_4666 = N_4584;
    out_arrsizze_4668 = nest_sizze_4636;
    if (memblock_set_device(ctx, &out_mem_4665, &mappingindices_mem_4650,
                            "mappingindices_mem_4650") != 0)
        return 1;
    if (memblock_set_device(ctx, &out_mem_4667, &mem_4656, "mem_4656") != 0)
        return 1;
    scalar_out_4659 = trend_4587;
    scalar_out_4660 = k_4588;
    scalar_out_4661 = n_4589;
    scalar_out_4662 = freq_4590;
    scalar_out_4663 = hfrac_4591;
    scalar_out_4664 = lam_4592;
    *out_scalar_out_4674 = scalar_out_4659;
    *out_scalar_out_4675 = scalar_out_4660;
    *out_scalar_out_4676 = scalar_out_4661;
    *out_scalar_out_4677 = scalar_out_4662;
    *out_scalar_out_4678 = scalar_out_4663;
    *out_scalar_out_4679 = scalar_out_4664;
    (*out_mem_p_4680).references = NULL;
    if (memblock_set_device(ctx, &*out_mem_p_4680, &out_mem_4665,
                            "out_mem_4665") != 0)
        return 1;
    *out_out_arrsizze_4681 = out_arrsizze_4666;
    (*out_mem_p_4682).references = NULL;
    if (memblock_set_device(ctx, &*out_mem_p_4682, &out_mem_4667,
                            "out_mem_4667") != 0)
        return 1;
    *out_out_arrsizze_4683 = out_arrsizze_4668;
    if (memblock_unref_device(ctx, &mem_4656, "mem_4656") != 0)
        return 1;
    if (memblock_unref_device(ctx, &out_mem_4667, "out_mem_4667") != 0)
        return 1;
    if (memblock_unref_device(ctx, &out_mem_4665, "out_mem_4665") != 0)
                       ...
                       struct futhark_f32_2d *in7)
{
    ...
    mappingindices_mem_4650.references = NULL;
    
    struct memblock_device images_mem_4651;
    
    images_mem_4651.references = NULL;
    ...
    hfrac_4591 = in4;
    lam_4592 = in5;
    mappingindices_mem_4650 = in6->mem;
    N_4584 = in6->shape[0];
    images_mem_4651 = in7->mem;
    m_4585 = in7->shape[0];
    N_4586 = in7->shape[1];
    
    int ret = futrts_main(ctx, &scalar_out_4659, &scalar_out_4660,
                          &scalar_out_4661, &scalar_out_4662, &scalar_out_4663,
                          &scalar_out_4664, &out_mem_4665, &out_arrsizze_4666,
                          &out_mem_4667, &out_arrsizze_4668,
                          mappingindices_mem_4650, images_mem_4651, N_4584,
                          m_4585, N_4586, trend_4587, k_4588, n_4589, freq_4590,
                          hfrac_4591, lam_4592);
    
    if (ret == 0) {
        *out0 = scalar_out_4659;
        *out1 = scalar_out_4660;
        *out2 = scalar_out_4661;
        *out3 = scalar_out_4662;
        *out4 = scalar_out_4663;
        *out5 = scalar_out_4664;
        assert((*out6 =
                (struct futhark_i32_1d *) malloc(sizeof(struct futhark_i32_1d))) !=
            NULL);
        (*out6)->mem = out_mem_4665;
        (*out6)->shape[0] = out_arrsizze_4666;
        assert((*out7 =
                (struct futhark_f32_1d *) malloc(sizeof(struct futhark_f32_1d))) !=
            NULL);
        (*out7)->mem = out_mem_4667;
        (*out7)->shape[0] = out_arrsizze_4668;
    }
    lock_unlock(&ctx->lock);
    return ret;
}
