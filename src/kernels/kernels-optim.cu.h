...
__global__ void ker1(uint N, int K, int freq, int* mappingindices, 
                                            float* X, float* XT){
	...
	if (gid < N*K) {
        ...
        if(rows == 0){
            ...
            double angle = 2 * PI * ((float)(rows / 2)) * ((float)mi_val) / freq;
            if(rows % 2 == 0) {
                res = sin(angle);
            } else {
                res = cos(angle);
...
// The completely naive version. 
__global__ void ker2naive(uint n, uint N, uint m, float* X, 
            float* XT, float* Y, float* Xsqr, uint K) {
...
template<int R>
__global__ void ker2simpletiled(uint n, uint N, uint m, float* X, 
            float* XT, float* YT, float* Xsqr, uint K) {
...
__global__ void matTransposeTiledKer(float* A, float* B, 
                uint heightA, uint widthA, int T) {
...
// The fully optimized version using tiling and shared memory.
template<int R> 
__global__ void ker2(uint n, uint N, uint m, float* X, float* XT, 
                        float* YT, float* Xsqr, uint K) {

    int ii  = blockIdx.x * R;       // grid.z
    int j1  = threadIdx.y;          // block.y
    int j2  = threadIdx.x;          // block.x
    // for copying to shared memory
...
// The completely naive version. 
__global__ void ker4naive(uint m, uint n, uint N, float* X, uint K, 
...
// Block-Tiling version without copying from global to shared memory.
__global__ void ker4simpletiled(uint m, uint n, uint N, float* X, 
                                uint K, float* Y, float* B0){
...
// The fully optimized version using tiling and shared memory. 
__global__ void ker4(uint m, uint n, uint N, float* X, 
                        uint K, float* Y, float* B0){
    ...
    for (uint kk = 0; kk < n; kk+=K) {        
        ...
        for(int k = 0; k < K; k++) {
            //pix_in is the local and k was originally (kk+k), but we look locally
            float y = Ysh[pix_in][k]; 

            if (y != F32_MIN) { 
                // X was indexed on i, and k because it is local
                acc += Xsh[i][k] * y; 
            } 
        }
        __syncthreads();
...
__global__ void ker5naive(uint m, float* Xinv, uint K, float* B0, float* B){
    uint pix  = blockIdx.x;
    uint i    = threadIdx.x;
    float acc = 0.0f;
...
// Block-Tiling version without copying from global to shared memory.
__global__ void ker5simpletiled(uint m, float* Xinv, uint K, float* B0, float* B){
...
__global__ void ker6simpletiled(uint m, uint N, float* XT, float* B, 
                                uint K, float* yhat){
    
    uint pix_out = blockIdx.y * blockDim.y;
    uint pix_in  = threadIdx.y;
    uint ii      = blockIdx.x * blockDim.x;
    uint i       = threadIdx.x;
    uint pix     = pix_out + pix_in;
    float acc    = 0.0f;
...
__global__ void ker6(uint m, uint N, float* XT, float* B, uint K, float* yhat){
    ...
    uint pix_out = blockIdx.y * T;
    uint pix_in  = threadIdx.y;
    uint ii      = blockIdx.x * T;
    uint i       = threadIdx.x;
    uint pix     = pix_out + pix_in;
    float acc    = 0.0f;

    __shared__ float Bsh [T][8+1];  // size K*K
    __shared__ float XTsh[T][8+1];  // size K*K
    ...
    if (ii+threadIdx.y < N && threadIdx.x < K) {
...
__global__ void ker7(uint m, uint N, float* yhat, uint* Nss, 
                    float* y_errors, float* Y, int* val_indss) {

    extern __shared__ volatile int shmem7[];
    volatile int*   shinds = (volatile int*)shmem7;
    volatile float* shvals = (volatile float*)(shinds + N); 

    uint  pix = blockIdx.x;     // grid.x = m
    uint  i   = threadIdx.x;    // blockDim.x = N
    float y   = Y[pix*N + i];
    float yh  = yhat[pix*N + i];

    float y_errors_i = F32_MIN;
    if (y != F32_MIN && !isnan(yh)) {
        y_errors_i = y-yh;
    }

    uint tf = 0;
    if (y_errors_i != F32_MIN && !isnan(y_errors_i)) {
        tf = 1; 
    }
    shinds[threadIdx.x] = tf;
    __syncthreads();
    ...
    // For result i, meaning it is the last thread in the block, 
    // holding the value of the reduce.
    if(threadIdx.x == N-1) {
        Nss[pix] = indT; 
    }
...
// The naive implementation of kernel 8. 
...
// The fully optimized version using shared memory. 
__global__ void ker8(uint m, uint n, uint N, uint K, float hfrac,
                          float* y_errors, float* Y, uint* nss, int* hs,
                          float* sigmas) {
    int pix = blockIdx.x;
    int i = threadIdx.x;
...
__global__ void ker9(float hfrac, uint n, uint m, uint N, int* hs, 
    ...
    extern __shared__ volatile float shmem9[];
    volatile float* sh_yerrs = (volatile float*)shmem9;
