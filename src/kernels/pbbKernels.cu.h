...
/**
 * A class representing a tuple of two ints, to be used
 * for partition. The reason for this is that we need
 * constructors (e.g., copy constructor) that makes it
 * work with the generic skeletons of reduce, scan, etc.
 */
// class MyInt2 {
...
//     __device__ __host__ inline MyInt2() {
//         x = 0; y = 0;
//     }
...
/**
 * Representation of the Partition2 operator, so that it can work the generically
 * defined skeletons of reduce, scan, etc.
 */
// class Partition2 {
//   public:
...
scanIncBlock(volatile typename OP::RedElTp* ptr, const unsigned int idx) {
    const unsigned int lane   = idx & (WARP-1);
    const unsigned int warpid = idx >> lgWARP; // idx/2^lgWARP -> 1/2^5=0,03125 ~ 0, 64/2^5=2 ...
...
copyFromGlb2ShrMem( const uint32_t glb_offs
    ...
    for(uint32_t i=0; i<CHUNK; i++) {
        ...
        // uint32_t loc_ind = threadIdx.x * CHUNK + i; // change line
