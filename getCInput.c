...
int futhark_entry_main(struct futhark_context *ctx, int32_t *out0,
                       ...
                       float *out5, int32_t *out6, int32_t *out7,
                       struct futhark_i32_1d **out8, const int32_t in0, const
...
static void futrts_cli_entry_main(struct futhark_context *ctx)
{
    ...
    int32_t read_value_3659;
    
    if (read_scalar(&i32_info, &read_value_3659) != 0)
        panic(1, "Error when reading input #%d of type %s (errno: %s).\n", 0,
              i32_info.type_name, strerror(errno));
    
    int32_t read_value_3660;
    
    if (read_scalar(&i32_info, &read_value_3660) != 0)
        panic(1, "Error when reading input #%d of type %s (errno: %s).\n", 1,
              i32_info.type_name, strerror(errno));
    
    int32_t read_value_3661;
    
    if (read_scalar(&i32_info, &read_value_3661) != 0)
        panic(1, "Error when reading input #%d of type %s (errno: %s).\n", 2,
              i32_info.type_name, strerror(errno));
    
    float read_value_3662;
    
    if (read_scalar(&f32_info, &read_value_3662) != 0)
        panic(1, "Error when reading input #%d of type %s (errno: %s).\n", 3,
              f32_info.type_name, strerror(errno));
    
    float read_value_3663;
    
    if (read_scalar(&f32_info, &read_value_3663) != 0)
        panic(1, "Error when reading input #%d of type %s (errno: %s).\n", 4,
              f32_info.type_name, strerror(errno));
    
    float read_value_3664;
    
    if (read_scalar(&f32_info, &read_value_3664) != 0)
        panic(1, "Error when reading input #%d of type %s (errno: %s).\n", 5,
              f32_info.type_name, strerror(errno));
    
    struct futhark_i32_1d *read_value_3665;
    int64_t read_shape_3666[1];
    int32_t *read_arr_3667 = NULL;
    
    errno = 0;
    if (read_array(&i32_info, (void **) &read_arr_3667, read_shape_3666, 1) !=
        0)
        panic(1, "Cannot read input #%d of type %s%s (errno: %s).\n", 6, "[]",
              i32_info.type_name, strerror(errno));
    
    struct futhark_f32_2d *read_value_3668;
    int64_t read_shape_3669[2];
    float *read_arr_3670 = NULL;
    
    errno = 0;
    if (read_array(&f32_info, (void **) &read_arr_3670, read_shape_3669, 2) !=
        0)
        panic(1, "Cannot read input #%d of type %s%s (errno: %s).\n", 7, "[][]",
              f32_info.type_name, strerror(errno));
    ...
    int32_t result_3677;
    int32_t result_3678;
    struct futhark_i32_1d *result_3679;
    ...
    if (perform_warmup) {
        ...
        ;
        ;
        assert((read_value_3665 = futhark_new_i32_1d(ctx, read_arr_3667,
                                                     read_shape_3666[0])) != 0);
        assert((read_value_3668 = futhark_new_f32_2d(ctx, read_arr_3670,
                                                     read_shape_3669[0],
                                                     read_shape_3669[1])) != 0);
        assert(futhark_context_sync(ctx) == 0);
        t_start = get_wall_time();
        r = futhark_entry_main(ctx, &result_3671, &result_3672, &result_3673,
                               &result_3674, &result_3675, &result_3676,
                               &result_3677, &result_3678, &result_3679,
                               read_value_3659, read_value_3660,
                               read_value_3661, read_value_3662,
                               read_value_3663, read_value_3664,
                               read_value_3665, read_value_3668);
        if (r != 0)
        ...
        ;
        ;
        assert(futhark_free_i32_1d(ctx, read_value_3665) == 0);
        assert(futhark_free_f32_2d(ctx, read_value_3668) == 0);
        ;
        ;
        ;
        ;
        ...
        ;
        ;
        assert(futhark_free_i32_1d(ctx, result_3679) == 0);
    }
    time_runs = 1;
    /* Proper run. */
    ...
    write_scalar(stdout, binary_output, &i32_info, &result_3671);
    printf("\n");
    write_scalar(stdout, binary_output, &i32_info, &result_3672);
    printf("\n");
    write_scalar(stdout, binary_output, &i32_info, &result_3673);
    printf("\n");
    write_scalar(stdout, binary_output, &f32_info, &result_3674);
    printf("\n");
    write_scalar(stdout, binary_output, &f32_info, &result_3675);
    printf("\n");
    write_scalar(stdout, binary_output, &f32_info, &result_3676);
    printf("\n");
    write_scalar(stdout, binary_output, &i32_info, &result_3677);
    printf("\n");
    write_scalar(stdout, binary_output, &i32_info, &result_3678);
    printf("\n");
    {
        ...
        assert(arr != NULL);
        assert(futhark_values_i32_1d(ctx, result_3679, arr) == 0);
        write_array(stdout, binary_output, &i32_info, arr,
                    futhark_shape_i32_1d(ctx, result_3679), 1);
        free(arr);
    }
    printf("\n");
    ;
...
static int futrts_main(struct futhark_context *ctx,
                       int32_t *out_scalar_out_3649,
                       int32_t *out_scalar_out_3650,
                       int32_t *out_scalar_out_3651, float *out_scalar_out_3652,
                       float *out_scalar_out_3653, float *out_scalar_out_3654,
                       int32_t *out_scalar_out_3655,
                       int32_t *out_scalar_out_3656,
                       struct memblock_device *out_mem_p_3657,
                       int32_t *out_out_arrsizze_3658,
                       struct memblock_device mappingindices_mem_3637,
                       struct memblock_device images_mem_3638, int32_t N_3626,
                       int32_t m_3627, int32_t N_3628, int32_t trend_3629,
                       int32_t k_3630, int32_t n_3631, float freq_3632,
                       float hfrac_3633, float lam_3634);
static inline int8_t add8(int8_t x, int8_t y)
{
    return x + y;
}
...
static int futrts_main(struct futhark_context *ctx,
                       ...
                       float hfrac_3633, float lam_3634)
{
    int32_t scalar_out_3639;
    int32_t scalar_out_3640;
    int32_t scalar_out_3641;
    float scalar_out_3642;
    float scalar_out_3643;
    float scalar_out_3644;
    int32_t scalar_out_3645;
    int32_t scalar_out_3646;
    struct memblock_device out_mem_3647;
    
    out_mem_3647.references = NULL;
    
    int32_t out_arrsizze_3648;
    
    out_arrsizze_3648 = N_3626;
    if (memblock_set_device(ctx, &out_mem_3647, &mappingindices_mem_3637,
                            "mappingindices_mem_3637") != 0)
        return 1;
    scalar_out_3639 = trend_3629;
    scalar_out_3640 = k_3630;
    scalar_out_3641 = n_3631;
    scalar_out_3642 = freq_3632;
    scalar_out_3643 = hfrac_3633;
    scalar_out_3644 = lam_3634;
    scalar_out_3645 = m_3627;
    scalar_out_3646 = N_3626;
    *out_scalar_out_3649 = scalar_out_3639;
    *out_scalar_out_3650 = scalar_out_3640;
    *out_scalar_out_3651 = scalar_out_3641;
    *out_scalar_out_3652 = scalar_out_3642;
    *out_scalar_out_3653 = scalar_out_3643;
    *out_scalar_out_3654 = scalar_out_3644;
    *out_scalar_out_3655 = scalar_out_3645;
    *out_scalar_out_3656 = scalar_out_3646;
    (*out_mem_p_3657).references = NULL;
    if (memblock_set_device(ctx, &*out_mem_p_3657, &out_mem_3647,
                            "out_mem_3647") != 0)
        return 1;
    *out_out_arrsizze_3658 = out_arrsizze_3648;
    if (memblock_unref_device(ctx, &out_mem_3647, "out_mem_3647") != 0)
        return 1;
    return 0;
}
struct futhark_f32_2d {
                       ...
                       struct futhark_f32_2d *in7)
{
    ...
    mappingindices_mem_3637.references = NULL;
    
    struct memblock_device images_mem_3638;
    
    images_mem_3638.references = NULL;
    ...
    hfrac_3633 = in4;
    lam_3634 = in5;
    mappingindices_mem_3637 = in6->mem;
    N_3626 = in6->shape[0];
    images_mem_3638 = in7->mem;
    m_3627 = in7->shape[0];
    N_3628 = in7->shape[1];
    
    int ret = futrts_main(ctx, &scalar_out_3639, &scalar_out_3640,
                          &scalar_out_3641, &scalar_out_3642, &scalar_out_3643,
                          &scalar_out_3644, &scalar_out_3645, &scalar_out_3646,
                          &out_mem_3647, &out_arrsizze_3648,
                          mappingindices_mem_3637, images_mem_3638, N_3626,
                          m_3627, N_3628, trend_3629, k_3630, n_3631, freq_3632,
                          hfrac_3633, lam_3634);
    
    if (ret == 0) {
        *out0 = scalar_out_3639;
        *out1 = scalar_out_3640;
        *out2 = scalar_out_3641;
        *out3 = scalar_out_3642;
        *out4 = scalar_out_3643;
        *out5 = scalar_out_3644;
        *out6 = scalar_out_3645;
        *out7 = scalar_out_3646;
        assert((*out8 =
                (struct futhark_i32_1d *) malloc(sizeof(struct futhark_i32_1d))) !=
            NULL);
        (*out8)->mem = out_mem_3647;
        (*out8)->shape[0] = out_arrsizze_3648;
    }
    lock_unlock(&ctx->lock);
    return ret;
}
